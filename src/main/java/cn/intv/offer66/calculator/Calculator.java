...
public class Calculator {
	...
	public static double conversion(final String expression) {
		...
		String newExpre;
		try {
			newExpre = transform(expression);
			result = cal.calculate(newExpre);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public double calculate(String expression) {
		...
		while (!postfixStack.isEmpty()) {
			...
			if (!isOperator(currentValue.charAt(0))) {// 如果不是运算符则存入操作数栈中
				...
				String tempResult = operating(firstValue, secondValue, currentValue.charAt(0));
	...
	// 按照给定的算术运算符做计算
	private String operating(String firstValue, String secondValue, char currentOp) {
		String result = "";
		switch (currentOp) {
		case '+':
